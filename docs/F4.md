...
A **module** is a plug‑in worker that automatically adds extra information—EXIF tags, OCR text, AI captions, thumbnails …—to every file you add to **Home‑Index**. Each module runs inside its own container and exchanges lightweight messages with the core service through **Redis queues**.

### What’s in it for you?

* **Richer, faster search** – the moment a module finishes, its new data is merged into `document.json` and is instantly searchable in Meilisearch.
* **Hands‑off operation** – modules wake up only when work is waiting and never block your usual sync.
* **Easy upgrades** – drop a new module into the list and Home‑Index will back‑fill your existing library without any manual steps.
...
All files on affected drives flip their marker to `‑status-pending` immediately, returning to `‑status-ready` once the new module versions finish (see F3).

### 1.3 Version tolerance

Modules **must remain forward‑compatible**: documents produced by older module versions stay searchable until the drive is remounted and re‑indexed. No module may crash because an archive has not yet been migrated (see F3).
...
3. On completion the module pushes a summary (done queue).
   ...
   * Home‑Index merges any `document` updates, updates Meilisearch, then (for archive files) re‑evaluates the drive marker (see F3).
4. Re‑building a module with a new UID triggers a re‑queue from that module onward; affected drives flip to `‑status-pending` (see F3).
...
`document.json` always contains a key called `"next"` (and so does every row in the Meilisearch index):

* The value is the **name of the next module** that still needs to process this file, e.g. `"text-module"`.
* It becomes an empty string `""` once the file has cleared the entire `MODULES` chain.

Use this field to build dashboards (e.g. *show all items still waiting for the vision module*) or to diagnose stuck files.
