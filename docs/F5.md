...
Modules can emit chunk documents with text segments. Each chunk is indexed with a sentence embedding vector. To search, embed your query text using the same model and POST it to the `file_chunks` index, including the `hybrid` parameter.
